									...
									<div class="card-body row">
										...
										<div class="col-md-6">
											...
											<h6 class="text-danger">
												{{ number_format($oferta->endeudamiento,0,'','.') }} €
											</h6>
										</div>
									</div>
									<hr class="m-0 verde" />
...
@section('scripts')
		...
		$(".oferta-detalle").on("hidden.bs.collapse", function(event) {
			$(".collapse-row")
				.not('.collapse-row[aria-expanded="true"]')
				.removeClass("bg-verde");
		});
		...
		$(".informacion").one("click", function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					if (data.status) {
						$boton
							.removeClass(["informacion", "btn-success"])
							.addClass("btn-info");
						$boton.html('{{ __("Información solicitada")}}');
						$boton.unbind("click");
						$("#titular-modal").html("Información solicitada");
						$("#texto-modal").html(
							"Has solicitado información de la oferta, en breve recibirás un correo de información y nos pondremos en contacto contigo."
						);
						$("#Modal").modal("show");
					} else {
						alert("Error");
