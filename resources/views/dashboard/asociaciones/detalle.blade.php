...
<div class="container-fluid">
			...
			<div class="card card-profile">
				...
				<div class="card-body   bg-gray">
					<h4 class="card-title text-white m-0">
						{{ e($asociacion->name) }} @if (!$asociacion->active)
						<span class="badge badge-danger float-right mr-2"> {{ __('Eliminada') }}</span> @endif @if ($nueva)
                ...
                <hr class=" m-0 ">
						...
						@endif @if ($asociacion->contactPhone)
						<small class="text-muted ">
                                    <i class="ti-mobile"></i>
                                    {{ __('Teléfono') }}
                                </small>
					...
					<div class=" tab-content">
						...
						<div class="tab-pane  @if($tab=='usuarios') active @endif" id="usuarios" role="tabpanel">
							...
							<div class="card-body bg-verde ">
								<h6 class="card-subtitle text-white m-0 ">{{ __('Listado de usuarios que pertenecen a la entidad.') }}</h6>
							</div>
								...
								<div class="table-responsive ">
									<table class="table  table-hover table-boder tabla-inversores">
										<tbody>
											...
											<tr>
												<td class="p-0">
													<a href="#collapse{{ $usuario_asociacion->id }}" class="tab-usuarios-link" role="button" aria-expanded="false" aria-controls="collapse{{ $usuario_asociacion->id }}"
											...
											<tr>
												<td colspan="5">
													<p>{{ __('No hay usuarios en la entidad.') }}</p>
												</td>
											</tr>
											@endif
										</tbody>
						...
						<div class="tab-pane  @if($tab=='modificar') active @endif" id="modificar" role="tabpanel">
							...
							<div class="card-body bg-verde ">
								<h6 class="card-subtitle text-white m-0 ">{{ __('Modifica los datos de la entidad.') }}</h6>
							</div>
									...
									<div class="form-group {{ $errors->has('address') ? ' has-danger' : '' }}">
										...
										<div class="col-md-12">
											<input type="text" value="{{ old('address')?old('address'): e($asociacion->address) }}" placeholder="{{ e($asociacion->address) }}"
											 class="form-control form-control-line {{ $errors->has('address') ? ' form-control-danger' : '' }}" id="address"
									...
									<div class="form-group {{ $errors->has('email') ? ' has-danger' : '' }}">
										...
										<div class="col-md-12">
											<input type="text" value="{{ old('email')?old('email'): e($asociacion->email) }}" placeholder="{{ e($asociacion->email) }}"
											 class="form-control form-control-line {{ $errors->has('email') ? ' form-control-danger' : '' }}" id="email" name="email">
										</div>
										@if ($errors->has('email'))
									...
									<div class="form-group {{ $errors->has('phone') ? ' has-danger' : '' }}">
										...
										<div class="col-md-12">
											<input type="text" value="{{ old('phone')?old('phone'): e($asociacion->phone) }}" placeholder="{{ e($asociacion->phone) }}"
											 class="form-control form-control-line {{ $errors->has('phone') ? ' form-control-danger' : '' }}" id="phone" name="phone">
										</div>
										@if ($errors->has('phone'))
						...
						<div class="tab-pane  @if($tab=='contacto') active @endif" id="contacto" role="tabpanel">
							...
							<div class="card-body bg-verde ">
								<h6 class="card-subtitle text-white m-0">{{ __('Modifica los datos de la persona de contacto de la entidad.') }}</h6>
							</div>
	...
	@include('dashboard.asociaciones.formulario.contacto')
						...
						<div class="tab-pane  @if($tab=='estado') active @endif" id="estado" role="tabpanel">
							...
							<div class="card-body bg-verde ">
								<h6 class="card-subtitle text-white m-0">{{ __('Cambia el estado de la entidad.') }}</h6>
							</div>
